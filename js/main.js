...
async function searchByChar(char) {
  ...
  // Ensure that the input value is limited to one character
  char = char.slice(0, 1);

  // Update the input field with the sanitized value
  document.querySelector(".one-char").value = char;
...
function displayCategoryData(data) {
  let box = ``;
  for (let i = 0; i < data.length; i++) {
    let description = data[i].strCategoryDescription
      .split(" ")
      .slice(0, 20)
      .join(" ");
    box += `
...
async function areaMealDetails(mealDetails) {
  ...
  const api = await fetch(
    `https://www.themealdb.com/api/json/v1/1/filter.php?a=${mealDetails}`
  );
  const response = await api.json();
  displayData(response.meals);
  $(".loading").addClass("d-none");
...
async function ingredientsMealDetails(mealDetails) {
  ...
  const api = await fetch(
    `https://www.themealdb.com/api/json/v1/1/filter.php?i=${mealDetails}`
  );
  const response = await api.json();
  displayData(response.meals);
  $(".loading").addClass("d-none");
...
function emailValidation() {
  ...
  const regex = /^\w.{0,20}@[a-z]{0,7}\.\w{2,5}$/gi;
  if (regex.test(input.value)) {
    $(input).addClass("is-valid");
    $(input).removeClass("is-invalid");
...
function phoneValidation() {
  ...
  const regex = /^(\+2)?(010|012|015|011)[0-9]{8}$/gi;
  if (regex.test(input.value)) {
    $(input).addClass("is-valid");
    $(input).removeClass("is-invalid");
